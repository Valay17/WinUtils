...
{
    // No lock: every window is created on, and every message dispatched to, the single UI thread.
        ...
        int height = 0)
    {
        ...
        // CreateWindowExW (WM_NCCREATE, WM_CREATE) go to DefWindowProc.
        Living[Handle] = this;
    }

    internal IntPtr Handle { get; private set; }

    // Return true and set result to stop the message here; false to pass it to DefWindowProc.
    ...
    private static IntPtr StaticWindowProcedure(IntPtr window, uint message, IntPtr wParam, IntPtr lParam)
    {
        ...
        {
            // Must not propagate - an exception crossing an UnmanagedCallersOnly boundary terminates the process.
    ...
    private static void EnsureClassRegistered(string className)
    {
        ...
        fixed (char* name = className)
        {
            var windowClass = new Win32.WNDCLASSEXW
            {
                ...
                // Without this, Windows never resets the cursor on entering the window.
