...
// A tray menu built from Win32 menu handles. TrackPopupMenu returns the
// integer id of whatever was chosen, so this class stores the callback
// against the id it hands out and invokes the right one itself. Submenus are
// owned by their parent - DestroyMenu on the root frees the whole tree.
internal sealed partial class PopupMenu : IDisposable
{
    // TrackPopupMenu returns 0 for "dismissed without choosing", which would
    // otherwise be indistinguishable from the first item.
    ...
    // TPM_RETURNCMD makes this synchronous: TrackPopupMenu runs its own
    // message loop and does not return until the menu closes.
    internal void ShowAndDispatch(IntPtr owner, int x, int y, uint alignment)
    {
        // Without this the menu will not dismiss on a click elsewhere - a menu
        // closes on losing activation, and one that was never activated
        // cannot lose it.
        ...
        // Documented requirement: the owner must receive a message after the
        // menu closes or the next click away is swallowed.
