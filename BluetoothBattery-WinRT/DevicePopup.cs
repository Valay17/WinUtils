...
internal sealed partial class DevicePopup : Win32Window
{
    ...
    private Windows.Devices.Radios.RadioState radioState_ = Windows.Devices.Radios.RadioState.Unknown;
    private bool refreshing_;
    private bool visible_;

    private PaintResources? paint_;

    // Asked for fresh each time rather than stored: the tray icon can move
    // (taskbar relocated, overflow flyout open/closed), and the shell reports
    // nothing while the flyout is closed.
    ...
    internal void ShowDevices(
        ...
        Windows.Devices.Radios.RadioState radioState,
        bool refreshing)
    {
    ...
    private static int HeightFor(int deviceCount, Windows.Devices.Radios.RadioState radioState)
    {
        if (radioState != Windows.Devices.Radios.RadioState.On)
        {
    ...
    private void Draw(Graphics g)
    {
        ...
        if (radioState_ != Windows.Devices.Radios.RadioState.On)
        {
    ...
    // Binary search on prefix length rather than trimming one character at a
    // time, which cost one MeasureString per character removed.
