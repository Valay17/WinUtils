...
internal static class Program
{
    ...
    internal static ProbeCache Cache { get; private set; } = new();
    ...
    private static int Main()
    {
        ...
        try
        {
            using (Timing.Measure("ProbeCache.Load"))
            {
                Cache = ProbeCache.Load();
            }
        ...
        {
            // Disposed here rather than left to the GC: this is the only
            // point guaranteed to be outside every message handler, and the
            // tray icon has to be removed explicitly.
            context?.Dispose();
...
internal sealed partial class MonitorContext : IDisposable
{
    ...
    internal MonitorContext()
    {
        ...
        // Created before the monitor, which captures the window handle in its
        // constructor - a handle cannot be replaced later the way a captured
        // SynchronizationContext could be.
        dispatcher_ = new UiDispatcher(window_.Handle);

        monitor_ = new BluetoothMonitor(dispatcher_);
        tray_ = new TrayController(window_.Handle, monitor_, RequestExit);
    ...
    // PostMessage rather than a direct call: the shutdown signal arrives on a
    // thread-pool thread, and PostQuitMessage only ends the loop of the
